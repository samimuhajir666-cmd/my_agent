...
# 7. If Whisper is not confident enough, return [unclear audio]
#    instead of pretending that a guessed word is correct.
#
# PIPELINE
# --------
# Microphone
...
def romanize_text(text: str) -> str:
    """
    Convert Urdu/Arabic script into approximate Latin/Roman text
    AFTER transcription.

    Important:
    We deliberately do NOT perform dangerous global replacements
    such as:
        N -> n
        aa -> a
        uu -> u

    Those replacements can corrupt valid English and Roman Urdu.
    """

    if not text:
        return ""

    # Already Latin / English / Roman Urdu.
    if not re.search(
        r"[^\x00-\x7F]",
        text,
    ):
    ...
    try:
        # unidecode is optional. If it is not installed, preserve the
        # model's real transcript rather than changing it incorrectly.
